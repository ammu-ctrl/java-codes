...
class Main{
    ...
    static class Bit {
        // Purpose: Get the k-th bit of x | Input: int x, int k | Output: int
        public static int get(int x, int k) { int res = (x >> k) & 1; DBG.log("Bit.get (" + x + "," + k + ")", res); return res; }
        // Purpose: Set the k-th bit of x | Input: int x, int k | Output: int
        public static int set(int x, int k) { int res = x | (1 << k); DBG.log("Bit.set (" + x + "," + k + ")", res); return res; }
        // Purpose: Toggle the k-th bit of x | Input: int x, int k | Output: int
        public static int toggle(int x, int k) { int res = x ^ (1 << k); DBG.log("Bit.toggle (" + x + "," + k + ")", res); return res; }
        // Purpose: Clear the k-th bit of x | Input: int x, int k | Output: int
        public static int clear(int x, int k) { int res = x & ~(1 << k); DBG.log("Bit.clear (" + x + "," + k + ")", res); return res; }
    }
